...
def cut_last_up(m, depth):
    ext = api.cast('IModelDocExtension', m.Extension); fm = api.cast('IFeatureManager', m.FeatureManager)
    m.ClearSelection2(True); ext.SelectByID2(last_sketch(m), 'SKETCH', 0,0,0, False, 0, None, 0)
    fc = fm.FeatureCut4(True, False, True, 0, 0, depth, depth, False, False, False, False, 0,0, False, False, False, False, False, True, True, True, True, False, 0, 0, False, False)
...
# 1) S30008: 블록(500, 0~20) -> 하부 컷 방향 반전(Dir=True, 깊이 15)
m,_ = resolve(app, DocSelector(path=os.path.join(D,'S30008MU0.SLDPRT'))); api.activate(app, m)
print('== S30008'); wipe(m)
sketch_on(m, lambda s: s.CreateCornerRectangle(-0.250,-0.250,0, 0.250,0.250,0))
print('  블록', extrude_last(m, 0.020))
sketch_on(m, lambda s: (s.CreateCornerRectangle(-0.260,-0.260,0, 0.260,0.260,0),
                        s.CreateCornerRectangle(-0.23825,-0.23825,0, 0.23825,0.23825,0),
                        s.CreateCornerRectangle(-0.23575,-0.23575,0, 0.23575,0.23575,0)))
print('  하부 컷(상향)', cut_last_up(m, 0.015))
report(m)
pdc = api.cast('IPartDoc', m); bx = [round(v*1000,1) for v in pdc.GetPartBox(True)]
mass = api.cast('IMassProperty', api.cast('IModelDocExtension', m.Extension).CreateMassProperty()).Mass
cover_ok = abs(mass - 10.6) < 1.5
print('  덮개 판+날 판정:', cover_ok, 'mass', round(mass,2))
# 2) S30009 경첩: 너클 중심=원점, 원점 안 지나는 프로파일 (z=-x_sk, y=y_sk)
m,_ = resolve(app, DocSelector(path=os.path.join(D,'S30009MU0.SLDPRT'))); api.activate(app, m)
print('== S30009 경첩'); wipe(m)
sketch_on(m, lambda s: s.CreateCircleByRadius(0, 0, 0, 0.004), plane='우측면')
print('  너클', extrude_last(m, 0.060))
sketch_on(m, lambda s: s.CreateCornerRectangle(0.002, -0.004, 0, 0.027, -0.002, 0), plane='우측면')
print('  윗날개', extrude_last(m, 0.060))
sketch_on(m, lambda s: s.CreateCornerRectangle(-0.004, -0.033, 0, -0.002, 0.002, 0), plane='우측면')
...
for n in list(C):
    if n.startswith('S30009MU0-'):
        ...
        if aext.SelectByID2(n + '@S30000MU0', 'COMPONENT', 0,0,0, False, 0, None, 0):
            print('경첩 삭제', n, aext.DeleteSelection2(0))
...
while f:
    if f.GetTypeName2()=='MateGroup':
        ...
        while s:
            ...
            if ec: dang.append(s.Name)
            s = api.cast('IFeature', s.GetNextSubFeature())
    f = api.cast('IFeature', f.GetNextFeature())
print('오류 메이트 삭제:', dang)
for nm in dang:
    am.ClearSelection2(True)
...
for tx in (-0.340, -0.195):
    a.AddComponents3(VARIANT(pythoncom.VT_ARRAY|pythoncom.VT_BSTR, [os.path.join(D,'S30009MU0.SLDPRT')]),
                     VARIANT(pythoncom.VT_ARRAY|pythoncom.VT_R8, [1,0,0, 0,1,0, 0,0,1, tx, 0.7765, 0.0105, 1,0,0,0]),
