...
# 흡수 안 된 스케치(실패한 컷1) 정리
f = api.cast('IFeature', m.FirstFeature()); loose=[]
while f:
    if f.GetTypeName2()=='ProfileFeature': loose.append(f.Name)
    f = api.cast('IFeature', f.GetNextFeature())
for nm in loose:
    m.ClearSelection2(True)
    if ext.SelectByID2(nm, 'SKETCH', 0,0,0, False, 0, None, 0):
        if ext.DeleteSelection2(1): print('스케치 정리', nm)
...
B = 0.23825
STRIPS = [
 ('W', (-0.255, -0.255, -B, 0.255), (-0.245, 0.0, 0.0)),
 ('E', (B, -0.255, 0.255, 0.255), (0.245, 0.0, 0.0)),
 ('N', (-B, B, B, 0.255), (0.0, 0.0, 0.245)),
 ('S', (-B, -0.255, B, -B), (0.0, 0.0, -0.245)),
]
for name, (x1, z1, x2, z2), pt in STRIPS:
    m.ClearSelection2(True)
    ok = ext.SelectByID2('', 'FACE', pt[0], pt[1], pt[2], False, 0, None, 0)
    skm.InsertSketch(True); skm.CreateCornerRectangle(x1, z1, 0, x2, z2, 0); skm.InsertSketch(True)
    ...
    print('컷', name, 'face', ok, 'cut', fc is not None)
    m.ForceRebuild3(False)
