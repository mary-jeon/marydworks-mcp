...
while f:
    if f.GetTypeName2() in ('Extrusion','Boss','ICE','Cut'): print('  feat', f.Name, f.GetTypeName2())
    f = api.cast('IFeature', f.GetNextFeature())
# 1) 덮개 날: 판 윗면(y5)에서 아래로 20 돌출(판 관통 병합)
...
pd = api.cast('IPartDoc', m)
b0 = [round(v*1000,1) for v in pd.GetPartBox(True)]
if b0[1] > -1:  # 날 없음
    m.ClearSelection2(True)
    ok = ext.SelectByID2('', 'FACE', 0.2, 0.005, 0.2, False, 0, None, 0); print('판 윗면 sel', ok)
    skm.InsertSketch(True)
    skm.CreateCornerRectangle(-0.23825,-0.23825,0, 0.23825,0.23825,0)
    skm.CreateCornerRectangle(-0.23575,-0.23575,0, 0.23575,0.23575,0)
    skm.InsertSketch(True)
    ...
    ff = fm.FeatureExtrusion3(True, False, True, 0, 0, 0.020, 0.0, False, False, False, False, 0.0, 0.0, False, False, False, False, True, True, True, 0, 0.0, False)
    print('날 돌출', ff is not None)
m.ForceRebuild3(False)
print('S30008 box', [round(v*1000,1) for v in pd.GetPartBox(True)], '| 바디', len(pd.GetBodies2(0, True) or []),
      '| mass %.3f' % api.cast('IMassProperty', ext.CreateMassProperty()).Mass)
print('save', write.save_model(app, m)['errors'])
# 2) 어셈블리: 끊긴 거리 메이트 삭제 -> 재생성
...
if aext.SelectByID2('거리11', 'MATE', 0,0,0, False, 0, None, 0): print('거리11 삭제', aext.DeleteSelection2(0))
...
for align, flip in [(1,False),(1,True),(0,False),(0,True)]:
    ...
    ok = (m2 is not None and err==1 and abs(b[4]-772.5)<0.3)
