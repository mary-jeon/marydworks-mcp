...
def last_sketch():
    ...
    while f:
        ...
        f = api.cast('IFeature', f.GetNextFeature())
    return last
def cut_from_bottom(draw, pt, depth):
    m.ClearSelection2(True)
...
# 컷1: 밑면에서 바깥 밴드(520/476.5) 깊이 15
ok, c1 = cut_from_bottom(lambda s: (s.CreateCornerRectangle(-0.260,-0.260,0, 0.260,0.260,0),
                                    s.CreateCornerRectangle(-0.23825,-0.23825,0, 0.23825,0.23825,0)),
                         (0.245, 0.0, 0.245), 0.015)
print('컷1(바깥 밴드) face', ok, 'cut', c1)
# 컷2: 밑면 안쪽(471.5) 깊이 15
ok, c2 = cut_from_bottom(lambda s: s.CreateCornerRectangle(-0.23575,-0.23575,0, 0.23575,0.23575,0),
                         (0.0, 0.0, 0.0), 0.015)
