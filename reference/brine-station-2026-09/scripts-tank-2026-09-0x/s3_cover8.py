...
def last_sketch():
    ...
    while f:
        ...
        f = api.cast('IFeature', f.GetNextFeature())
    return last
def extrude(depth, down=False, merge=True):
    m.ClearSelection2(True); ext.SelectByID2(last_sketch(), 'SKETCH', 0,0,0, False, 0, None, 0)
    ff = fm.FeatureExtrusion3(True, False, down, 0, 0, depth, 0.0, False, False, False, False, 0.0, 0.0, False, False, False, False, merge, True, True, 0, 0.0, False)
    return ff is not None
print('== S30008 멀티바디'); wipe()
...
skm.InsertSketch(True)
ok_down = extrude(0.015, down=True, merge=False)
print('날(하향, merge=False)', ok_down)
rotated = False
if not ok_down:
    ok_up = extrude(0.020, down=False, merge=False)
    print('날(상향 폴백, merge=False)', ok_up)
    rotated = ok_up
m.ForceRebuild3(False)
...
if mass < 10 or mass > 11.5: sys.exit('형상 이상 — 중단')
# 어셈블리
