# 2026-09-09 밤: 대공사 3b단계 — 사용자 「긴 러그(126) 없는 구조로 실린더 선정」: 스트로크를 220으로 길게 잡아 설치길이(339)로 판 사이 거리(355)를 채움 → 후단 러그 26. 실사용 신장 120은 중간 리미트 스위치(기능 2)로.
# 원본: — 사용자 「브래킷이 안 생기게 실린더를 선정하라」
...
STROKE_ORD=220.0                  # 주문 스트로크(설치길이 확보용)
TA2_RETRACT=STROKE_ORD+119.0      # 339
...
while dd:
    ...
    dd=nx
# ================= 1) TA2 B9e (양단 클레비스 U) =================
P_TA2=Zp("B9f_TiMOTION_TA2-2H-220_24V_clevisU_mid120.SLDPRT")
if not os.path.exists(P_TA2):
    ...
    d.Extension.SelectByID2(last_sketch(d),"SKETCH",0,0,0,False,0,NOD,0); f=cut_blind(d,tube_end-112.0,True,"튜브_보어_D21")
    if not any(bx[2]<=-tube_end+1 for bx in cyl_r(d,10.5)):
        delete_feat(d,"튜브_보어_D21"); d.Extension.SelectByID2(last_sketch(d),"SKETCH",0,0,0,False,0,NOD,0); cut_blind(d,tube_end-112.0,False,"튜브_보어_D21")
    print("  bore faces",cyl_r(d,10.5))
    ...
    props(d,{"TITLE":"LINEAR ACTUATOR TiMOTION TA2 (500 N, 양단 클레비스 U)","SPEC":f"TiMOTION TA2-2H-{STROKE_ORD:.0f}: 24 V DC, 하중코드 H(500 N 밀기/당기기, 셀프락 500 N, 17/14 mm/s), 주문 스트로크 {STROKE_ORD:.0f}(실사용 {STROKE:.0f} — 중간 리미트 스위치 위치 {STROKE:.0f} 지정, 기능 2), 설치길이(후퇴, 홀-홀) {TA2_RETRACT:.0f}=스트로크+119(151~300 구간 +2 포함 여부 승인도면 확인). 후단 취부 5(AL CNC 클레비스 U, 홈 6·깊이 16·홀 Ø8)·전단 취부 5(AL 주조 클레비스 U, 홈 6·깊이 10.5·홀 Ø8), 후단 방향 0°, 리미트 스위치 1(양단 전류 차단), 출력신호 0, IP66D, 케이블 1000. 사용온도 −25~+65 ℃","MATERIAL":"AL casting/SUS rod","QT'Y":"1","REMARK":"설치길이를 판 사이 거리(고정판 밑면 −10 ↔ 로드 핀 −365 = 355)에 맞추려고 스트로크를 220으로 길게 주문 — 후단 핀이 판 밑 16에 와서 러그가 26으로 줄어듦. 양단이 실린더 자체 클레비스 U라 스테이션 쪽은 평판 러그 PL6(J8e·J9d)만 용접. 로봇 커버 실린더 TA2-2H-200과 같은 제조사·코드. 3D는 데이터시트 근사(클레비스 U 바깥폭 18/20 가정) — 승인도면으로 확정"})
    saveas(d,P_TA2)
...
def make_lug(path,z_lo,z_hi,hole_z,title,spec,remark):
    ...
    saveas(d,path)
make_lug(Zp("J8e_lug_PL6_40x26.SLDPRT"),-10.0,-Z_REAR_PIN-10.0,0.0,"TA2 REAR LUG (FLAT PL6)",
...
NEW={"B9f_TiMOTION_TA2-2H-220_24V_clevisU_mid120.SLDPRT":(I3,(AX,0,Z_REAR_PIN),{"상승":1,"하강":1,"1.상승했을때(해석)":0,"2.하강했을때(해석)":0}),
     "J8e_lug_PL6_40x26.SLDPRT":(I3,(AX,0,Z_REAR_PIN),{"상승":1,"하강":1,"1.상승했을때(해석)":1,"2.하강했을때(해석)":0}),
...
for n in list(cc):
    if n.startswith(("B9e_TiMOTION","J8d_lug")):
...
for cfg in CFGS:
    ...
    for fn,(R,t,supp) in NEW.items():
        ...
        if fn.startswith("B9f") and supp[cfg]: cc=comps(); cc[n].ReferencedConfiguration=("하강" if cfg.startswith(("하강","2.")) else "상승")
...
for cfg in CFGS:
    ...
    if cfg in ("상승","하강"):
        for n in act_:
            if n.startswith(("B9f","J8e","J9d","G11e","J11d")): print(f"  [{cfg}] {n:40s} {box(cc[n])}")
        a.ClearSelection2(True)
...
json.dump(rep,open(os.path.join(VER,"ta2_rebuild_lug2_0909.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1)
