...
# ---- J8e 러그 높이 26 → 23 (TA2 후단 U 홈 바닥 = 홀 아래 7)
d=act(Zp("J8e_lug_PL6_40x26.SLDPRT"))
prm=d.Parameter("D1@러그_PL6"); print("J8e D1",prm.SystemValue*1000)
if abs(prm.SystemValue*1000-26)<0.1: prm.SystemValue=mm(23.0); d.EditRebuild3
print("J8e box",[round(v*1000,1) for v in pv(d,"GetPartBox",True)],"hole",cyl_r(d,4.0))
cp=d.Extension.CustomPropertyManager(""); cp.Set2("SPEC","STS304 PL6 × 40 × 23(파일명 26은 구치수), 핀홀 Ø8(하단에서 7). TA2 후단 클레비스 U(홈 6·깊이 16)에 삽입"); save(d)
# ---- 라인 간섭
...
LINE=("B9f","J8e","J9d","G11e","J11d","G3b","B4b","J19e","J5e","J1c","J2_","B10","G13","G14","H16","J17")
for cfg in ("상승","하강"):
    ...
    for n,c in line:
        base=n.split("/")[-1]
        if base.startswith(("J17","J5e","B9f","B4b","J19e","B10","J8e")):
...
json.dump(rep,open(os.path.join(VER,"ta2_rebuild_check2_0909.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1)
