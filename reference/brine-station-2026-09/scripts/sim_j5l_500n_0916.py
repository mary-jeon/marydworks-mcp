# 2026-09-16: SOLIDWORKS Simulation 재실행 — 32A 라인 현 형상(J1c+J8g t5.8 / J5l t8+J9f PL6 h69), TA2-2H-85 loadpush/pull 500 N(TraceParts 원문). sim_j5e_500n_0910.py에서 부품명·구멍 반경(소켓 Ø49·부시 Ø28 H7)만 바꿈.
...
FIXED_KEEP=["J1c_fixed_plate_185x580_t10-2","J8g_lug_PL5.8_40x23_pin16-1"]
DN_KEEP=["J5l_moving_plate_180x540_t8-1","J9f_lug_PL6_40x69_pin60-1"]
cw=None
...
report={"force_N":FORCE_N,"date":"2026-09-16","parts":{"up":FIXED_KEEP,"dn":DN_KEEP},"material":"AISI 304 (SW lib)","assumption":"용접=본딩, 고정판 상면 고정, 소켓 구멍 고정(노즐 걸림), 부시 구멍 반경 구속"}
...
add_fixed(s1,top,"고정판 상면(호퍼 립 용접면)"); add_force_z(s1,pin,ref,+FORCE_N,"J8g 핀홀 TA2 500 N 상향"); add_gravity(s1,ref)
...
sock=cyl_faces(mplate,24.5); bush=cyl_faces(mplate,14.0); pin2=cyl_faces(lug2,4.0); print("  study2 faces: socket",len(sock),"bush",len(bush),"lug pin",len(pin2)); assert sock and bush and pin2
s2,doc=new_study("2_DN_static"); ref=plane_ref(); set_materials(s2)
add_fixed(s2,sock,"소켓 구멍 Ø49(노즐 걸림)"); add_slider_xy(s2,bush,"부시 구멍 반경 구속"); add_force_z(s2,pin2,ref,-FORCE_N,"J9f 핀홀 TA2 500 N 하향"); add_gravity(s2,ref)
if mesh_and_run(s2): report["2_DN_static"]=results(s2,"2.하강")
asm.ShowConfiguration2("상승"); asm.EditRebuild3
out=os.path.join(VER,"J5l_sim_results_500N_0916.json"); json.dump(report,open(out,"w",encoding="utf-8"),ensure_ascii=False,indent=1); print("saved",out,"; assembly NOT saved")
stop.set()
