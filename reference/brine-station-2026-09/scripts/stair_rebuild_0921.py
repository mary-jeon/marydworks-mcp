# 2026-09-21 계단 단축(사용자 지시: 계단이 바디프레임 후단 z -2836을 넘지 않게, 8단 유지, 발 280): RUN 270->205(50.1도), 디딤판 300->280(하단 플랜지 간섭 회피). stair_rebuild_0917.py 사본, 기존 파일은 백업 후 SaveAs 덮어쓰기.
...
def insert(path,t):
    ...
    return c.Name2
'''
ASM20=Zp("S20000MU0.SLDASM"); BK=r"<MCP_DIR>\_backup\20260921-stair"
...
# ---------- 0. 구 계단 컴포넌트를 어셈블리에서 빼고 파트 문서 닫기(파일은 그대로, 뒤에서 SaveAs 덮어쓰기) ----------
if STAGE in ("all","asmdel"):
    ...
    for p in (P_PL,P_PLL,P_TR,P_RA,P_MR):
...
json.dump(rep,open(os.path.join(VER,f"stair_rebuild_0921_{STAGE}.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str)
print("DONE",STAGE)
