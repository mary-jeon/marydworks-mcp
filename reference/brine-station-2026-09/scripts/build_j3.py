# J3 = J2 + LA25 mounting fixed: 6T rear U-bracket with pin holes (J8b), rod clevis on the moving plate top (J9) + pin (J11),
# moving plate without Ø40 hole (J5c). Fixed stack / hose / rods unchanged from J2. Memory only — no save.
import os, json, math
from swconn import *
stop=watchdog()
app=connect()
...
R_LA25_J=[[-1,0,0],[0,-1,0],[0,0,1]]
R_UH_J=[[0,1,0],[0,0,1],[1,0,0]]         # U parts: u->y, v->+z, depth(part -z)->x
R_PIN_J=[[1,0,0],[0,0,1],[0,-1,0]]       # pin along +y
R_HOSEUP=[[1,0,0],[0,0,1],[0,-1,0]]
# ---- layout (line coords; fixed plate top z=0 = 지상고 2,050)
H_RAISE=140.0
VB=-118.0; NIP_FIX_END=VB-30.0
RX=60.0; RY=240.0; ACT_Y=300.0; OX=80.0; STROKE=140.0
ZP_UP=-380.0
CLV_H=40.0; CLV_PIN=25.0; CLV_W=42.0     # clevis on plate top: pin at zp+25
ZEYE=ZP_UP+CLV_PIN; tz_la=ZEYE+161.5; ZBACK=58.5+tz_la     # -355 / -193.5 / -135
BR_W=136.0; BR_H=194.0; BR_HOLE=69.0
assert abs((-10-BR_H+BR_HOLE)-ZBACK)<1e-6, ("bracket hole vs ZBACK", -10-BR_H+BR_HOLE, ZBACK)
...
F("B9_LINAK_LA25_2500N_150st_24V.SLDPRT",R_LA25_J,(0,ACT_Y,tz_la)); print("  LA25 back eye z",ZBACK,"rod eye z",ZEYE)
F("J8b_bent_U_bracket_t6_136x194x60.SLDPRT",R_UH_J,(30,ACT_Y+30-BR_W/2,-10-BR_H))   # legs centred on housing centre y=330
F("G11_clevis_pin_d10_L160.SLDPRT",R_PIN_J,(0,ACT_Y+30-80,ZBACK))
asm.EditRebuild3
...
def moving(tag,dz):
    ...
    M("J5c_moving_plate_200x620_t10.SLDPRT",I,(0,0,zp))
    M("G13_weld_socket_3-4in_L25.SLDPRT",I,(OX,0,zp+15))
    M("H16_hose_nipple_3-4in_short_L30.SLDPRT",I,(OX,0,zp+45))
    M("J17_pipe_3-4in_L100.SLDPRT",I,(OX,0,zp))
    M("B10_linear_bushing_MISUMI_LHFRW16.SLDPRT",R_BUSH,(RX,RY,zp)); M("B10_linear_bushing_MISUMI_LHFRW16.SLDPRT",R_BUSH,(RX,-RY,zp))
    M("J9_rod_clevis_t6_42x40x60.SLDPRT",R_UH_J,(30,ACT_Y-CLV_W/2,zp))          # u->y 279..321, v->z zp..zp+40
    M("J11_clevis_pin_d10_L70.SLDPRT",R_PIN_J,(0,ACT_Y-35,zp+CLV_PIN))
    P1=[0.0,0.0,NIP_FIX_END]; P2=[OX,0.0,zp+45]
    if tag=="up":
        M("J19b_hose_3-4in_up_r40.SLDPRT",R_HOSEUP,(0,0,NIP_FIX_END))
    else:
        L=math.hypot(P2[0]-P1[0],P2[2]-P1[2]); u=[(P2[0]-P1[0])/L,0,(P2[2]-P1[2])/L]
        M(straight,R_dir(u),(P1[0],0,P1[2]))
        M("B12_LA25_rod_extension_d20_L140.SLDPRT",I,(0,ACT_Y,ZEYE))               # ZEYE-140..ZEYE = 하강 아이 위치
    return names
...
res={"params":{"H_RAISE":H_RAISE,"VB":VB,"RX":RX,"RY":RY,"ACT_Y":ACT_Y,"OX":OX,"ZP_UP":ZP_UP,"STROKE":STROKE,"ZEYE":ZEYE,"ZBACK":ZBACK,"tz_la":tz_la,"BR_H":BR_H,"CLV_PIN":CLV_PIN},"fixed":fixed,"up":up,"dn":dn}
for cfg in ("상승","하강"):
...
asm.ShowConfiguration2("상승"); asm.EditRebuild3
json.dump(res,open(os.path.join(VER,"J3_rebuild_boxes.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1)
print("components:",len(res["상승"]),"-> _검증/J3_rebuild_boxes.json ; NOT SAVED (dirty=%s)"%asm.GetSaveFlag)
stop.set()
