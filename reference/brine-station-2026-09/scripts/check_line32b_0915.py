# 2026-09-15(B9h 교체 후): 32A 라인 검사 — 라인 자체 간섭(상승/하강) + 스테이션 S00000MU0 라인↔로봇(커버군)↔탱크 간섭·지상고. 읽기 전용(저장 안 함)
import os, sys, json
...
rep={}
LINE=("B9h","J23b","J8g","J9f","G11f","J11e","G3e","B4e","G13f","G13g","H16d","J19i","J5l","J1c","J2c","B10")
# ---- 라인
...
for cfg in ("상승","하강"):
    ...
    for n,c in line:
        base=n.split("/")[-1]
        if base.startswith(("G13f","J19i","J5l","B9h","B4e","G3e","H16d","G13g","J9f")): g[base]=gl(box(c))
    for n,c in out:
        base=n.split("/")[-1]
...
json.dump(rep,open(os.path.join(VER,"check_line32c_lidopen_0915.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str)
