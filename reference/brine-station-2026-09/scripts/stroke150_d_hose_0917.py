# 2026-09-17 5단계(4단계 x −45안은 하강 시 노즐이 로봇 뚜껑과 4.4 겹침 → 폐기): 나선 고리 호스, 나가는 다리 오프셋 +y 45(소켓 (0,45)), 고리는 −x(앞) 쪽, 상단 직선 50(KE005 밑 여유)
#   이동판 J5m(210×540, 구멍 (−45,0)) → J5n(180×540, 구멍 (0,45))
...
PITCH=45.0; S1=50.0; RC_UP=(190.0-41.0)/2; XOFF=0.0; YOFF=PITCH; EX,EY=-1.0,0.0   # 나가는 다리 (0,+45), 고리 방향 −x
...
P_J5M=Zp("J5m_moving_plate_210x540_t8.SLDPRT"); P_J5N=Zp("J5n_moving_plate_180x540_t8.SLDPRT")
P_UP=Zp("J19n_hose_YASUNG_HSPF-032_up_loop190.SLDPRT"); P_DN=Zp(f"J19n_hose_YASUNG_HSPF-032_dn_loop{2*RC_DN+41:.0f}.SLDPRT")
...
def build_hose(path,Rc,s2,D,title,spec_add,rebuild=False):
    ...
    set_props(d,{"TITLE":title,"SPEC":HOSE_SPEC+spec_add,"Material":"PVC","QT'Y":"1","DATE":DATE,"REMARK":"구매품(야성판매). 사용자 지시 「접힌 고리 폭 상승 시 190」 반영. 고리는 나선 1회전(−x 앞쪽, 피치 45를 +y로 φ−sinφ 분배해 양단 접선 수직), 나가는 다리 (0,+45)(이동판 소켓 위치). 실물은 자중·강선 탄성으로 형상이 달라질 수 있음."},mat="PVC 경질")
    if rebuild:
...
if STAGE in ("all","parts"):
    ...
    build_hose(P_DN,RC_DN,S2_DN,D_DN,f"HOSE 32A (하강 상태, 고리 폭 {2*RC_DN+41:.0f}) — YASUNG HSPF-032",
        f" 절단 = 자유길이 {LF:.0f} + 바브 2×{STUB:.0f} ≈ {L_HOSE:.0f}. 하강(낙차 {D_DN:.1f}): 바브 + 직선 {S1:.0f} + 나선 1회전 Rc {RC_DN:.1f}(−x 쪽, 피치 +y {PITCH:.0f}, 고리 바깥 폭 {2*RC_DN+41:.0f}) + 직선 {S2_DN:.0f} + 바브. Rc = 내경의 {RC_DN/32:.2f}배.")
    # 이동판 J5m → J5n: 외곽 x −75 → −45(180×540), 소켓 Ø49 (−45,0) → (0,45)
    if not os.path.exists(P_J5N):
        d=act(P_J5M)
        ...
        for s_ in list(pv(sk,"GetSketchSegments") or []):
            ty=s_.GetType() if callable(s_.GetType) else s_.GetType
            if ty==0: s_.Select4(True,NOD); n+=1
            elif ty==1:
                cx,cy,r=circ_xy(s_)
                if abs(r-24.5)<0.01: s_.Select4(True,NOD); n+=1
        d.Extension.DeleteSelection2(0); print("  J5m deleted segs",n)
        sm=d.SketchManager; sm.AddToDB=True
        sm.CreateCornerRectangle(mm(-45),mm(-270),0,mm(135),mm(270),0); sm.CreateCircleByRadius(mm(XOFF),mm(YOFF),0,mm(24.5)); sm.AddToDB=False
        try:
        ...
        bx=bbox(d); print("  J5n box",bx,"ww",ww(d),"orphans",orphan_sketches(d)); assert bx==[-45.0,-270.0,-8.0,135.0,270.0,0.0] and not ww(d) and not orphan_sketches(d), bx
        pr=get_props(d); sp=pr.get("SPEC","").replace("210×540(x −75~135, y ±270)","180×540(x −45~135, y ±270)").replace("Ø49(−45,0)","Ø49(0,45)")
        set_props(d,{"TITLE":pr.get("TITLE","").replace("210","180") if pr.get("TITLE") else "MOVING PLATE 180x540 t8","SPEC":sp,"DATE":DATE,"REMARK":"자작(판재 절단·탭). 호스 고리(나선) 나가는 다리 (0,+45)에 맞춰 소켓 위치 이동."})
        e=I4(); w=I4(); ok=d.Extension.SaveAs(P_J5N,0,1,NOD,e,w); print("  SaveAs J5n",ok,e.value,w.value); assert ok
        rep["J5n_spec"]=sp
# ---------- 2. 어셈블리 ----------
...
if STAGE in ("all","asm"):
    ...
    J5=[n for n in cc if n.startswith("J5n_")]; assert J5, [n for n in cc if n.startswith("J5")]; J5=J5[0]; print("plate comp",J5)
    tj=xform(cc[J5])["t_mm"]; assert abs(tj[2]-ZP_UP)<0.02, tj
    # 이동측 소켓·노즐·호스니플: x 거리 45 → 일치(0), y 일치 → 거리 45
    MOV={"G13g_socket_Rc1-1-4_ONDA_SFS3-32_STEP-1":"이동_G13g-1","G13f_barrel_nipple_R1-1-4_ONDA_SFN2-32_STEP-2":"이동_G13f-2","H16d_hose_nipple_R1-1-4x34_ONDA_SFHN-3234_STEP-2":"이동_H16d-2"}
    EXIST=set(mate_names())
    for comp,tag in MOV.items():
        x0=xform(cc[comp]); t_exp=[XOFF,YOFF,x0["t_mm"][2]]
        for axis,off in ((0,XOFF),(1,YOFF)):
            mname=f"{tag}_{'xyz'[axis]}"; del_mate(mname); a.EditRebuild3; done=False
            variants=[(0,False),(1,False)] if abs(off)<1e-6 else [(0,False),(0,True),(1,False),(1,True)]
            for al,fl in variants:
                a.ClearSelection2(True); assert sel_plane(comp,axis,False) and sel_plane(J5,axis,True)
                ok,e,f=add_mate(0,al,False,0,mname) if abs(off)<1e-6 else add_mate(5,al,fl,abs(off)/1000,mname)
                if not ok: continue
                g,x=xf_ok(comp,x0["R"],t_exp if axis==1 else [XOFF,x0["t_mm"][1],x0["t_mm"][2]])
                if g and not ww(a): done=True; break
                del_mate(mname)
            assert done,(comp,mname)
        print(f"  {comp[:40]} -> {xform(comps()[comp])['t_mm']}")
        EXIST=set(mate_names())
    # 호스: J19k 삭제 → J19m 삽입(원점 (0,0,−154.6), R 단위)
    cc=comps(); old=[n for n in cc if n.startswith("J19m_")]; print("delete",old)
    ...
    for p,tag in ((P_UP,"고정_J19nup"),(P_DN,"고정_J19ndn")):
...
if STAGE in ("all","asm","verify"):
    ...
    for cfg in ("상승","하강"):
        ...
        for n,(t,b,s) in sorted(row.items()):
            if s==2 and n.startswith(("J19n","J5n","G13f","G13g","H16d","F4")): print(f"   {n[:46]:46s} t={t} box={b}")
    a.ShowConfiguration2("상승"); a.ForceRebuild3(False)
    json.dump(out,open(os.path.join(VER,"stroke150d_asm_positions_0917.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str)
if STAGE in ("all","asm","interf"):
    ...
    a.ShowConfiguration2("상승"); a.ForceRebuild3(False)
json.dump(rep,open(os.path.join(VER,f"stroke150d_0917_{STAGE}.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str)
print("DONE",STAGE)
