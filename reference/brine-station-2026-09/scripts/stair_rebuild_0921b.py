# 2026-09-21b 측판 ㄷ→ㅁ 박스 250x50x4.5T, 지주를 박스 윗면 위에 올림(x ±325, 하단 경사 절단). RUN 205·디딤판 280 유지, 난간대 S20017·S20018·디딤판 S20015는 기존 파일 재사용. stair_rebuild_0917.py 사본, 백업 후 SaveAs 덮어쓰기.
...
mm=lambda v:v/1000.0; Zp=lambda n: os.path.join(Z,n); DATE="2026-09-21"
...
def insert(path,t):
    ...
    return c.Name2
'''
ASM20=Zp("S20000MU0.SLDASM"); BK=r"<MCP_DIR>\_backup\20260921-stair-box"
GROUND=-9.0; PLAT_Y=1953.0; PLAT_Z=980.0; N=8; RUN=205.0; RISE=(PLAT_Y-GROUND)/N; TH=math.atan2(RISE,RUN)
...
# ---------- 0. 구 계단 컴포넌트를 어셈블리에서 빼고 다시 만들 파트 문서 닫기 ----------
if STAGE in ("all","asmdel"):
    ...
    exec(ASM_HELPERS)
    cc=comps(); old=[n for n in cc if n.split("/")[-1].startswith(("S20014","S20015","S20016","S20017","S20018","S20019"))]
    print("remove from asm",len(old),old)
    ...
    for p in (P_PL,P_PLL,P_PO):
        bk=os.path.join(BK,os.path.basename(p)); assert os.path.exists(bk) and os.path.getsize(bk)==os.path.getsize(p),("backup missing",p)
        if app.GetOpenDocumentByName(p) is not None: app.CloseDoc(p)
        assert app.GetOpenDocumentByName(p) is None,("still open",p)
# ---------- 1. 파트 ----------
if STAGE in ("all","parts"):
    ...
    def build_channel(path,inward,title_side):
        ...
        def draw(sm):
            xf=sgn*FL_W
            for pts in ([(0,0),(xf,0),(xf,-PL_H),(0,-PL_H)],[(sgn*PL_T,-PL_T),(sgn*(FL_W-PL_T),-PL_T),(sgn*(FL_W-PL_T),-(PL_H-PL_T)),(sgn*PL_T,-(PL_H-PL_T))]):
        ...
        area=PL_H*FL_W-(PL_H-2*PL_T)*(FL_W-2*PL_T); v=volume(d); assert abs(v-area*LEN)/(area*LEN)<0.002, (v,area*LEN)
        ...
        set_props(d,{"TITLE":f"STAIR STRINGER (절곡 ㅁ 박스, {title_side})","SPEC":f"절곡 ㅁ 박스 PL 4.5T STS304: 250 × 50(판재를 사각형으로 접어 이음매 1줄 용접, 판금 R = t/2 = 2.25 권장), 안쪽 면 x 300 · 바깥 면 x 350, 경사 {math.degrees(TH):.1f}°(단높이 {RISE:.2f}×{N}, 단너비 {RUN:.0f}), 윗면 = 앞코선 +20, 경사길이 ≈{L:.0f}, 하단 수평 절단(지면), 상단 수직 절단(플랫폼 기둥 S20002 앞면 z 980). 윗면(폭 50)에 지주 Ø48.6을 올려 용접, 안쪽 평면에 디딤판 용접.","Material":"STS304","QT'Y":"1","DATE":DATE,"REMARK":"자작(레이저 절단 후 절곡·이음매 용접). ㄷ채널(09-17) 대체 — 사용자 지시(ㅁ 판재, 봉은 위에 올림). 비틀림 J ≈3.9e6 mm4(ㄷ 1.0e4)."},mat="AISI 304")
    ...
    POST_OFF=(POST_D/2)*TAN+5.0; POST_L=POST_OFF+RAIL_H-PL_TOP_OFF-(RAIL_D/2)/COS   # 원점 = 축선, 박스 윗면 접점보다 POST_OFF 아래
    if True:
        d=app.NewDocument(tmpl,0,0,0)
        ...
        for conv in (0,1):
            assert sel_plane_p(d,"우측면"); d.SketchManager.InsertSketch(True); skobj=d.SketchManager.ActiveSketch; sm=d.SketchManager; sm.AddToDB=True
            pts=[(0,POST_OFF-200*TAN,-200),(0,POST_OFF+200*TAN,200),(0,-300,200),(0,-300,-200)]; sp=[sketch_xy(skobj,mm(q[0]),mm(q[1]),mm(q[2]),conv) for q in pts]
            for i in range(4): sm.CreateLine(sp[i][0],sp[i][1],0,sp[(i+1)%4][0],sp[(i+1)%4][1],0)
            sm.AddToDB=False; d.SketchManager.InsertSketch(True); d.ClearSelection2(True)
            skc=[n for n,t in feats(d) if t=="ProfileFeature"][-1]; d.Extension.SelectByID2(skc,"SKETCH",0,0,0,False,0,NOD,0)
            v0=volume(d); c=d.FeatureManager.FeatureCut4(False,False,False,1,1,0.0,0.0,False,False,False,False,0.0,0.0,False,False,False,False,False,True,True,False,False,False,0,0.0,False,False); d.EditRebuild3
            good=False
            if c is not None and len(bodies(d))==1:
                bx=bbox(d); com=[v*1000 for v in d.Extension.CreateMassProperty.CenterOfMass]
                print(f"   post miter conv {conv} dv {round(v0-volume(d))} box {bx} com_z {com[2]:.3f}")
                good=abs(bx[1]-5.0)<0.1 and abs(bx[4]-POST_L)<0.05 and com[2]<-0.01
            if good: c.Name="하단_경사컷"; ok=True; break
            if c is not None: c.Select2(False,0); d.EditDelete()
            clean_orphans(d)
        assert ok,"post miter"
        set_props(d,{"TITLE":"STAIR HANDRAIL POST","SPEC":f"Ø48.6×3.7T STS304 (KS D 3576 40A Sch40) 수직, 하단 {math.degrees(TH):.1f}° 경사 절단 — 측판(ㅁ 250×50) 윗면 위에 올려 전둘레 용접, 축선 길이 {POST_L-POST_OFF:.1f}(윗면 접점 ~ 난간대 하면). 제13조 7호 100 kg 캔틸레버(레버 880) σ ≈158 MPa(내력 205, SF 1.3).","Material":"STS304","QT'Y":"8","DATE":DATE,"REMARK":"자작. 측판 옆면 부착(09-17) → 윗면 올림 — 사용자 지시."},mat="AISI 304")
...
if STAGE in ("all","asm"):
    ...
    for k in (1,3,5,7):
        z,y=nos(k)
        for sx,side in ((1,"R"),(-1,"L")): plan.append((P_PO,[sx*POST_X,y+PL_TOP_OFF-((POST_D/2)*TAN+5.0),z],f"지주{k}{side}"))
...
json.dump(rep,open(os.path.join(VER,f"stair_rebuild_0921b_{STAGE}.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str)
print("DONE",STAGE)
