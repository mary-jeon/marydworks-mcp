# 2026-09-15: TraceParts TA2-2H-085339-5511-010-1 STEP → B9g 규약(후단 핀 원점, 축 −Z, 핀 축 Y, 모터 +X)으로 정렬 → 구성 상승/하강(로드 −85) → B9h 저장·닫기
import os, sys, json
...
STEP=os.path.join(BASE,r"_원문\32A\timotion\actuator_ta2-2h-085339-5511-010-1_-_hubst0.stp")
OUT=os.path.join(Z,"B9h_TiMOTION_TA2-2H-085339-5511-010-1.SLDPRT"); assert not os.path.exists(OUT), OUT
STROKE=85.0; RL=339.0; mm=lambda v:v/1000.0
stop=watchdog(); app=connect()
already=[x for x in (pv(app,"GetDocuments") or []) if x.GetTitle.startswith("actuator_ta2-2h-085339")]
...
props={"TITLE":"LINEAR ACTUATOR TiMOTION TA2 (500 N, 스트로크 85, 설치길이 339, 양단 클레비스 U)",
 "SPEC":f"TiMOTION TA2-2H-085339-5511-010-1: 24 V DC, 하중코드 H(500 N 밀기/당기기, 셀프락 500 N, 17/14 mm/s, 1.3/0.8 A — TraceParts 속성 원문), 스트로크 85, 설치길이(후퇴, 홀-홀) 339 지정, 후단 취부 5·전단 취부 5(클레비스 U, 홈 6·홀 Ø8), 리미트 스위치 1, 출력신호 0, IP 없음(코드 0), 케이블 100. 3D = TraceParts 제조사 구성 STEP(REFERENCE TA2-2H-085339-5511-010-1) 원형 — 클레비스 U 바깥폭 실측 후단 {U_rear:.1f}·전단 {U_front:.1f}(승인도면 확인). 파트 좌표: 후단 핀 원점, 축 −Z, 핀 축 Y, 모터 +X",
 "MATERIAL":"AL casting/SUS rod","QT'Y":"1","DATE":"2026-09-15",
 "REMARK":"B9g(스트로크 140 대용)를 발주 형번 85로 교체. 구성 하강 = 로드 바디 −85 이동(STEP은 후퇴 상태만 제공).",
...
json.dump(rep,open(os.path.join(VER,"ta2_b9h_build_0915.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str)
stop.set(); print("B9h build done")
