# 2026-09-17(스트로크 150·4점 Ø20·U190 호스 반영 후, check_line32b_0915 기반): 32A 라인 검사 — 라인 자체 간섭(상승/하강) + 스테이션 S00000MU0 라인↔로봇(커버군)↔탱크 간섭·지상고. 읽기 전용(저장 안 함)
import os, sys, json
...
rep={}
LINE=("B9j","J23d","J8g","J9f","G11f","J11e","G3e","B4e","G13f","G13g","H16d","J19n","J5n","J1c","J2d","B10b","F4")
# ---- 라인
...
ctrl=any(any(x.startswith("G11f") for x in r[0]) and any(x.startswith("B9j") for x in r[0]) for r in rep["line_상승"])
print("양성 대조(핀↔TA2 클레비스 U 겹침 검출):",ctrl); rep["positive_control"]=ctrl
# ---- 스테이션
...
for cfg in ("상승","하강"):
    ...
    for n,c in line:
        base=n.split("/")[-1]
        if base.startswith(("G13f","J19n","J5n","B9j","B4e","G3e","H16d","G13g","J9f","J2d","B10b","J23d")): g[base]=gl(box(c))
    for n,c in out:
        base=n.split("/")[-1]
...
json.dump(rep,open(os.path.join(VER,"check_stroke150d_station_0917.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str)
