# 2026-09-17 계단 보정: 지주 길이(난간대 하면 = 축 − r/cosθ) · 난간대 끝 플랫폼 기둥 앞면(z 980) 수직 절단 → 간섭 재검
...
from swconn import *
from swpv import pv
exec(open(os.path.join(os.path.dirname(os.path.abspath(__file__)),"stair_rebuild_0917.py"),encoding="utf-8").read().split("# ---------- 0. 백업")[0])
...
cp=d.Extension.CustomPropertyManager(""); sp=cp.Get("SPEC").replace(f"L{POST_BOT_BELOW+RAIL_H-RAIL_D/2:.1f}",f"L{POST_L2:.1f}"); cp.Set2("SPEC",sp)
...
json.dump({"post_L":POST_L2,"interf":res},open(os.path.join(VER,"stair_fix_0917.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str); print("DONE fix")
