# 2026-09-22: TraceParts TA2-2H-150314-5511-010-1 STEP(RL 274 -> 314, 스트로크 150 고정, 사용자 지시 「실린더를 더 긴 것」) → 같은 규약(후단 핀 원점, 축 −Z, 핀 축 Y, 모터 +X) → 구성 상승/하강(로드 −150) → B9k 저장·닫기 (ta2_85_build_0915 기반)
import os, sys, json
...
STEP=os.path.join(BASE,r"_원문\32A\timotion\actuator_ta2-2h-150314-5511-010-1_-_hubst0.stp")
OUT=os.path.join(Z,"B9l_TiMOTION_TA2-2H-150314-5511-010-1.SLDPRT"); assert not os.path.exists(OUT), OUT
STROKE=150.0; RL=314.0; mm=lambda v:v/1000.0
stop=watchdog(); app=connect()
already=[x for x in (pv(app,"GetDocuments") or []) if x.GetTitle.startswith("actuator_ta2-2h-150314")]
...
props={"TITLE":"LINEAR ACTUATOR TiMOTION TA2 (500 N, 스트로크 150, 설치길이 314, 양단 클레비스 U)",
 "SPEC":f"TiMOTION TA2-2H-150314-5511-010-1: 24 V DC, 하중코드 H(500 N 밀기/당기기, 셀프락 500 N, 17/14 mm/s, 1.3/0.8 A — TraceParts 속성 원문), 스트로크 150(표준 범위 20~150), 설치길이(후퇴, 홀-홀) 314 지정(최소 269 = 150+119보다 45 길게, 데이터시트 20160711-M p.6은 하한만 규정), 후단 취부 5·전단 취부 5(클레비스 U, 홈 6·홀 Ø8), 리미트 스위치 1, 출력신호 0, IP 없음(코드 0), 케이블 100. 3D = TraceParts 제조사 구성 STEP(REFERENCE TA2-2H-150314-5511-010-1) 원형 — 클레비스 U 바깥폭 실측 후단 {U_rear:.1f}·전단 {U_front:.1f}(승인도면 확인). 파트 좌표: 후단 핀 원점, 축 −Z, 핀 축 Y, 모터 +X",
 "MATERIAL":"AL casting/SUS rod","QT'Y":"1","DATE":"2026-09-22",
 "REMARK":"구매품. B9k(RL 274)를 RL 314로 교체 — 이동판 행정을 40 내리기 위해(스트로크 150 고정, 러그 연장 대신 실린더 길게). 구성 하강 = 로드 바디 −150 이동(STEP은 후퇴 상태만 제공). 주문 표기: TA2-2H-150, Retracted Length 314, 후단 5·전단 5, 방향 90°(코드 1), 리미트 1, 신호 0.",
...
json.dump(rep,open(os.path.join(VER,"ta2_b9l_build_0922.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=1,default=str)
stop.set(); print("B9l build done")
